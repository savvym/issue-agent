...
### 1) `GITHUB_TOKEN (or GH_TOKEN) is required`

需要在服务端环境变量或设置页里提供 GitHub Token。

### 2) `Bad Request` / provider 报错

优先检查：

- `baseURL` 是否正确（某些网关需要 `/v1`）
- `apiType` 与网关能力是否匹配（`responses` 或 `chat`）
- `model` 是否可用

### 3) 为什么报告有时代码依据偏弱？

当前是“采样式证据采集”（Code Search + TopN 文件片段），并非全仓全文索引。  
如果检索命中少、权限不足、或仓库结构特殊，代码证据会减少。
